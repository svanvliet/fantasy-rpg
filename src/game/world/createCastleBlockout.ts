...
export interface AssetSwapAnchor {
  id: string;
  mount: THREE.Group;
  hiddenFallbackObjects: THREE.Object3D[];
  ghostFallbackObjects: THREE.Object3D[];
}
...
function addWallSconce(
  ...
  const backplate = new THREE.Mesh(
    new THREE.BoxGeometry(0.14, 0.46, 0.05),
    new THREE.MeshStandardMaterial({
      color: 0x493426,
      roughness: 0.76,
      metalness: 0.22
  ...
  (backplate.material as THREE.MeshStandardMaterial).envMapIntensity = 0.8;
  ...
  (arm.material as THREE.MeshStandardMaterial).envMapIntensity = 0.72;
  arm.position.set(0, -0.03, 0.14);
  fixture.add(arm);
  ...
  (brace.material as THREE.MeshStandardMaterial).envMapIntensity = 0.68;
  brace.rotation.x = -Math.PI / 6;
  brace.position.set(0, -0.11, 0.08);
  fixture.add(brace);

  const cup = new THREE.Mesh(
    new THREE.CylinderGeometry(0.07, 0.09, 0.05, 8),
    new THREE.MeshStandardMaterial({
      color: 0x8b6848,
      roughness: 0.72,
      metalness: 0.08
  ...
  (cup.material as THREE.MeshStandardMaterial).envMapIntensity = 0.52;
  cup.position.set(0, -0.03, 0.26);
  fixture.add(cup);

  const candle = new THREE.Mesh(
    new THREE.CylinderGeometry(0.036, 0.04, 0.22, 8),
    new THREE.MeshStandardMaterial({
      color: 0xe7dcc8,
      roughness: 0.88,
      metalness: 0.02
  ...
  (candle.material as THREE.MeshStandardMaterial).envMapIntensity = 0.25;
  candle.position.set(0, 0.07, 0.26);
  fixture.add(candle);

  const wick = new THREE.Mesh(
    new THREE.CylinderGeometry(0.006, 0.006, 0.028, 6),
    new THREE.MeshStandardMaterial({
      color: 0x20160f,
      roughness: 0.9,
      metalness: 0.01
  ...
  wick.position.set(0, 0.195, 0.26);
  fixture.add(wick);

  const ember = new THREE.Mesh(
    new THREE.SphereGeometry(0.028, 8, 8),
    new THREE.MeshBasicMaterial({ color: 0xffe3b2 })
  );
  ember.position.set(0, 0.24, 0.26);
  fixture.add(ember);

  switch (facing) {
    case "north":
      fixture.rotation.y = Math.PI;
      fixture.position.z -= 0.025;
      break;
    case "east":
      fixture.rotation.y = -Math.PI / 2;
      fixture.position.x += 0.025;
      break;
    case "west":
      fixture.rotation.y = Math.PI / 2;
      fixture.position.x -= 0.025;
  ...
  addPointLight(scene, {
    ...
    intensity: 5.4,
    distance: 5.2,
    position: new THREE.Vector3(position.x, position.y + 0.24, position.z),
    castShadow: false,
    showEmber: false
  });
  ...
  addBounceLight(
    ...
    1.05,
    4.2,
    new THREE.Vector3(position.x, position.y - 0.18, position.z)
  );
}

function addCeilingBeam(
  scene: THREE.Scene,
  world: RAPIER.World,
  rapier: typeof RAPIER,
  centerX: number,
  ...
  addStaticBox(scene, world, rapier, {
    color: 0x3a281d,
    position: new THREE.Vector3(centerX, 4.05, z),
    scale: new THREE.Vector3(9.4, 0.22, 0.34),
    roughness: 0.9,
    metalness: 0.02,
    collider: false
  });
...
function addBed(
  ...
  parts.push(addStaticBox(scene, world, rapier, {
    ...
    scale: new THREE.Vector3(2.08, 0.36, 2.72)
  }));
  parts.push(addStaticBox(scene, world, rapier, {
    color: 0x85735d,
    position: new THREE.Vector3(position.x, position.y + 0.34, position.z - 0.04),
    scale: new THREE.Vector3(1.9, 0.12, 2.34),
    collider: false
  }));
  parts.push(addStaticBox(scene, world, rapier, {
    color: 0x47301f,
    position: new THREE.Vector3(position.x, position.y + 0.86, position.z - 1.3),
    scale: new THREE.Vector3(2.18, 0.9, 0.14)
  }));
  return parts;
}
...
function addPartitionWall(
  ...
  addStaticBox(scene, world, rapier, {
    color: wallColor,
    position: new THREE.Vector3(x, 4.18, 0),
    scale: new THREE.Vector3(0.8, 0.26, 5.86)
  });
  addStaticBox(scene, world, rapier, {
    color: 0x3a2c22,
    position: new THREE.Vector3(x, 4.38, 0),
    scale: new THREE.Vector3(0.8, 0.06, 5.98),
    collider: false,
    castShadow: false
  });
}
...
function addRug(scene: THREE.Scene, position: THREE.Vector3, scale: THREE.Vector3, color: number): void {
  ...
  (rug.material as THREE.MeshStandardMaterial).envMapIntensity = 0.12;
...
function createAssetSwapAnchor(
  ...
  return {
    id,
    mount,
    hiddenFallbackObjects,
    ghostFallbackObjects
  };
}
...
export function createCastleBlockout(
  ...
  const ambientLight = new THREE.AmbientLight(0xf1dcc2, 0.48);
  scene.add(ambientLight);

  const fillLight = new THREE.HemisphereLight(0xaaa2b1, 0x2c2117, 0.3);
  scene.add(fillLight);
  ...
  [-4.4, -0.4, 3.6].forEach((z) => {
    addCeilingBeam(scene, world, rapier, -10, z);
    addCeilingBeam(scene, world, rapier, 0, z);
    addCeilingBeam(scene, world, rapier, 10, z);
  });
  ...
  addTable(scene, world, rapier, new THREE.Vector3(-7.95, 0.95, -4.15), new THREE.Vector3(1.25, 0.75, 0.95), 0x654d36);
  addStaticBox(scene, world, rapier, {
    color: 0x6a5238,
  ...
  const bedsideCandles = addCandles(
    ...
    rapier,
    [
      new THREE.Vector3(-8.1, 0.95, -4.08),
      new THREE.Vector3(-7.72, 0.95, -4.25),
      new THREE.Vector3(-10.12, 0.9, -0.92)
    ],
    4.8,
    3.4
  );

  addShelf(scene, world, rapier, new THREE.Vector3(-1.6, 0, -4.85), new THREE.Vector3(1.5, 2.7, 0.7), 0x5f4731, 4);
  addShelf(scene, world, rapier, new THREE.Vector3(1.45, 0, -4.75), new THREE.Vector3(1.5, 2.5, 0.7), 0x5a4330, 3);
  const cabinetBody = addStaticBox(scene, world, rapier, {
    color: 0x5a4330,
    position: new THREE.Vector3(2.45, 1.3, 5.05),
    scale: new THREE.Vector3(1.8, 2.6, 0.9)
  });
  const cabinetDoorLeft = addStaticBox(scene, world, rapier, {
    color: 0x6e563d,
    position: new THREE.Vector3(2.16, 1.3, 4.591),
    scale: new THREE.Vector3(0.79, 2.288, 0.04),
    collider: false
  });
  const cabinetDoorRight = addStaticBox(scene, world, rapier, {
    color: 0x6e563d,
    position: new THREE.Vector3(2.74, 1.3, 4.591),
    scale: new THREE.Vector3(0.79, 2.288, 0.04),
    collider: false
  });
  addStaticBox(scene, world, rapier, {
    color: 0x72543b,
    position: new THREE.Vector3(-3.0, 0.6, 4.25),
    scale: new THREE.Vector3(1.1, 1.2, 1.1)
  });
  addStaticBox(scene, world, rapier, {
    color: 0x695238,
    position: new THREE.Vector3(-1.1, 0.5, 4.15),
  ...
  addCandles(
    ...
    rapier,
    [
      new THREE.Vector3(-1.18, 2.7, -4.45),
      new THREE.Vector3(1.72, 2.5, -4.32),
      new THREE.Vector3(2.45, 2.6, 4.68)
    ],
    4.2,
    3.1
  );

  const alchemyTablePosition = new THREE.Vector3(10, 0.95, -0.1);
  ...
  (alchemyBottle1.material as THREE.MeshStandardMaterial).envMapIntensity = 0.92;
  alchemyBottle1.position.set(9.65, alchemyBottleCenterY, 0.02);
  ...
  (alchemyBottle2.material as THREE.MeshStandardMaterial).envMapIntensity = 0.92;
  alchemyBottle2.position.set(10.1, alchemyBottleCenterY, -0.08);
  ...
  (alchemyBottle3.material as THREE.MeshStandardMaterial).envMapIntensity = 0.92;
  alchemyBottle3.position.set(10.6, alchemyBottleCenterY, 0.02);
  ...
  (alchemyBottle4.material as THREE.MeshStandardMaterial).envMapIntensity = 0.92;
  alchemyBottle4.position.set(11.08, alchemyBottleCenterY, 0.24);
  ...
  const alchemyBoard = addStaticBox(scene, world, rapier, {
    color: 0x493325,
    position: new THREE.Vector3(alchemyTablePosition.x, alchemyTableSurfaceY + 0.03, alchemyTablePosition.z),
    scale: new THREE.Vector3(2.36, 0.08, 1.12),
    collider: false
  });
  addShelf(scene, world, rapier, new THREE.Vector3(12.65, 0, 4.65), new THREE.Vector3(1.3, 2.5, 0.7), 0x5f4732, 4);
  addTable(scene, world, rapier, new THREE.Vector3(7.55, 0.85, 4.2), new THREE.Vector3(1.6, 0.7, 1), 0x684d34);
  addStaticBox(scene, world, rapier, {
    color: 0x6c5236,
    position: new THREE.Vector3(7.45, 1.1, -4.55),
  ...
  addCandles(
    ...
    rapier,
    [
      new THREE.Vector3(9.18, 0.95, -0.46),
      new THREE.Vector3(10.82, 0.95, 0.26),
      new THREE.Vector3(7.28, 0.85, 4.15),
      new THREE.Vector3(12.55, 2.5, 4.78)
    ],
    5.2,
    3.6
  );
  ...
  addPointLight(scene, {
    color: 0xffbe78,
    intensity: 18,
    position: new THREE.Vector3(10, 2.85, -0.05),
    distance: 10
  });
  addPointLight(scene, {
    color: 0xffd8a0,
  ...
  addWallSconce(scene, new THREE.Vector3(-12.35, SCONCE_HEIGHT, -5.55), "south");
  addWallSconce(scene, new THREE.Vector3(-8.25, SCONCE_HEIGHT, -5.55), "south");
  addWallSconce(scene, new THREE.Vector3(-1.85, SCONCE_HEIGHT, 5.55), "north");
  addWallSconce(scene, new THREE.Vector3(2.05, SCONCE_HEIGHT, 5.55), "north");
  addWallSconce(scene, new THREE.Vector3(8.1, SCONCE_HEIGHT, 5.55), "north");
  addWallSconce(scene, new THREE.Vector3(12.15, SCONCE_HEIGHT, 5.55), "north");
  ...
  (focalArch.material as THREE.MeshStandardMaterial).envMapIntensity = 0.48;
  focalArch.rotation.z = Math.PI / 2;
  focalArch.scale.setScalar(0.82);
  focalArch.position.set(10.02, 1.72, -0.08);
  ...
  assetSwapAnchors.push(
    ...
    createAssetSwapAnchor(
      ...
      [...alchemyTableBlockout, focalArch],
      [alchemyBoard]
    ),
